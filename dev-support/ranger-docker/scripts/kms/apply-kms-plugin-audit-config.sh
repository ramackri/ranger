...
#
# Point KMS ranger-kms-audit.xml at the audit ingestor (Kerberos).
#
# Usage (from dev-support/ranger-docker):
#   ./scripts/kms/apply-kms-plugin-audit-config.sh
#   ./scripts/kms/apply-kms-plugin-audit-config.sh --check-only
#   ./scripts/kms/apply-kms-plugin-audit-config.sh --no-restart
...
NO_RESTART=false
AUDIT_URL_OVERRIDE=""
KMS_AUDIT_XML="${RANGER_HOME:-/opt/ranger}/kms/ews/webapp/WEB-INF/classes/conf/ranger-kms-audit.xml"
...
if [[ -f /opt/ranger/kms/install.properties ]]; then
  INSTALL_PROPS=/opt/ranger/kms/install.properties
  IN_CONTAINER=true
elif [[ -f /opt/ranger/ranger-kms-plugin/install.properties ]]; then
  INSTALL_PROPS=/opt/ranger/ranger-kms-plugin/install.properties
  IN_CONTAINER=true
else
  INSTALL_PROPS="${SCRIPT_DIR}/ranger-kms-install-postgres.properties"
  IN_CONTAINER=false
fi
...
AUDIT_SPOOL="${AUDIT_SPOOL:-/var/log/kms/audit/audit-ingestor/spool}"
...
updates = {
    ...
    'xasecure.audit.jaas.Client.option.serviceName': prop('XAAUDIT.JAAS.CLIENT.OPTION.SERVICE_NAME', 'kms'),
    'xasecure.audit.jaas.Client.option.keyTab': prop('XAAUDIT.JAAS.CLIENT.OPTION.KEY_TAB', '/etc/keytabs/rangerkms.keytab'),
    'xasecure.audit.jaas.Client.option.principal': prop('XAAUDIT.JAAS.CLIENT.OPTION.PRINCIPAL', 'rangerkms/ranger-kms.rangernw@EXAMPLE.COM'),
}
...
PY
  mkdir -p "${AUDIT_SPOOL}"
  chown -R rangerkms:ranger "${AUDIT_SPOOL}" 2>/dev/null || chown -R rangerkms:ranger /var/log/kms/audit 2>/dev/null || true
}
...
run_in_container() {
  if [[ ! -f "${KMS_AUDIT_XML}" ]]; then
    echo "WARN: ${KMS_AUDIT_XML} not found; KMS setup may not have run yet" >&2
    return 1
  fi
  if [[ "${CHECK_ONLY}" == "true" ]]; then
    audit_ok "${KMS_AUDIT_XML}"
    return $?
  fi
  apply_audit_xml "${KMS_AUDIT_XML}"
  echo "Applied auditserver config to ${KMS_AUDIT_XML}"
}
...
if [[ "${CHECK_ONLY}" == "true" ]]; then
  docker exec "${CONTAINER}" bash -c "RANGER_HOME=/opt/ranger ${RANGER_SCRIPTS:-/home/ranger/scripts}/apply-kms-plugin-audit-config.sh --check-only"
  exit $?
fi

docker exec "${CONTAINER}" bash -c "RANGER_HOME=/opt/ranger ${RANGER_SCRIPTS:-/home/ranger/scripts}/apply-kms-plugin-audit-config.sh"
if [[ "${NO_RESTART}" != "true" ]]; then
  echo "Restarting Ranger KMS in ${CONTAINER}..."
  docker exec "${CONTAINER}" bash -c 'cd /opt/ranger/kms && ./ranger-kms-services.sh stop' 2>/dev/null || true
  sleep 5
  docker exec "${CONTAINER}" bash -c 'cd /opt/ranger/kms && ./ranger-kms-services.sh start' 2>/dev/null || true
  for _ in $(seq 1 24); do
    if curl -sf -o /dev/null \
      "http://localhost:9292/kms/v1/keys/names?user.name=${KMS_E2E_USER:-keyadmin}" 2>/dev/null; then
      echo "Ranger KMS is responding on 9292."
      exit 0
    fi
    sleep 5
  done
  echo "WARN: Ranger KMS may not be ready; check: docker logs ${CONTAINER}" >&2
fi
