...
# E2E-only: widen dev_ozone audit filters so allowed volume/bucket creates are audited.
# Default dev_ozone filters audit DENIED only. Idempotent.
...
if any(f.get("description") == marker for f in filters):
    print(f"dev_ozone: audit filter '{marker}' already present")
else:
    ...
    print(f"dev_ozone: prepended E2E audit-all filter ({len(filters)} filters total)")
PY
