...
| REST GET/PUT | Yes | Yes (PUT) | AdminClient grow only |
...
### Topic size + `AdminClient.createPartitions` (reuse for REST PUT)
...
| **`PartitionPlanService`** | Orchestrates GET/PUT: validate, allocate, AdminClient, registry write |
...
| `AuditMessageQueueUtils.updateExistingTopicPartitions()` | same | REST PUT → grow `ranger_audits` |
...
This is intentionally simple for **rare** reads (startup, REST GET with force-read, compare-and-swap before PUT).
...
|-----------|-------------------------------|-----------------|
| Write rate | Rare (ops: bootstrap, promote, scale) | Very high (every audit) |
...
| `version` | Monotonic integer; optimistic locking on REST PUT |
...
After this one-time publish, **all routing and later changes** use the plan in Kafka (REST or promote/scale), not XML edits.
...
**Alternative:** wait for first admin REST PUT instead of auto-publish — possible, but auto-publish is recommended for pod 2+.
...
| REST `PUT` / promote / scale | **Yes** | Read current version, then produce new version |
...
1. Finds unassigned tail partition IDs (or increases topic first).
2. Adds `trino` to `plan.plugins` with new IDs.
...
| 4 | Validate + allocate (promote / scale) |
...
```mermaid
flowchart TD
  Admin[Admin PUT] --> Auth[Auth + authorize]
...
sequenceDiagram
  ...
  PodA->>PlanTopic: produce v5 (scale hive)
...
Two pods both scaling from v4 might temporarily compute overlapping tail IDs in memory — only one produce wins the compare-and-swap. The loser gets **409** and must **re-read v5** and allocate from the **new tail**, not reuse its stale allocation. Append-only validation on PUT rejects plans that steal IDs already assigned in the current version.
...
## Worked example: hdfs + hiveServer2, then promote trino

**Initial plan (v1)** — from bootstrap or first PUT:
...
| Partial write (partitions increased but plan not written) | Retry PUT idempotently; allocator sees higher topic count |
| Two concurrent PUTs (different pods) | `expectedVersion` + re-read before produce + read-back → one **200**, one **409**; client retries on latest version |
...
flowchart LR
  ...
  P3 --> P4[4 REST GET]
  P4 --> P5[5 REST PUT promote scale]
...
| `createPartitions` succeeds but plan write fails | Medium | Increase audit topic **before** publishing plan; retry PUT idempotently |
...
4. **PUT validation:** optional strict mode — reject writes that shrink or reassign existing plugin partition IDs.
